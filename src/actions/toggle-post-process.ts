...
@action({ UUID: "pl.lukaszpodgorski.handy.toggle-post-process" })
export class TogglePostProcess extends SingletonAction<Settings> {
  override async onKeyDown(ev: KeyDownEvent<Settings>): Promise<void> {
    ...
    try {
      spawn(binary, ["--toggle-post-process"], {
        detached: true,
        stdio: "ignore",
      }).unref();
